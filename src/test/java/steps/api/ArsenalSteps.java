...
public class ArsenalSteps {
    ...
    @Then("Verify broadband content should be visible for unsubscribed CP {string} only")
    public void verifyBroadbandContentShouldBeVisibleForUnsubscribedCPOnly(String arg0) {
        for (int i = 0; i < version1Collections.getContents().size(); i++) {
            Assert.assertEquals(arg0, version1Collections.getContents().get(i).getExtras().get("_cpId"));
        }
    }
}
