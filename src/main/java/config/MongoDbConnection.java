...
public class MongoDbConnection {
    ...
    static MongoCollection<Document> segmentsCollection,clientPolicyCollection ,tNcCollection, clientDetailCollection, activeClientCollection,clientPolicy2Collection, accountCollection;

    public static void connectWithDataBase(String databaseName) {
    ...
    public static void getSegmentCollection() {
        logger.info("******************************** segments Detail ************************************************");
        segmentsCollection = database.getCollection("segments");
    }
    ...
    //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    public static List<Document> getSegmentById(String segmentId) {
        List<Document> Segments = segmentsCollection.find(eq("_id",new ObjectId(segmentId))).into(new ArrayList<Document>());
        return Segments;
    }
