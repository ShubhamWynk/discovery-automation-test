...
## Executing the tests
To run the sample project, run the JUnit runner class or use Maven as the primary build tool. Use the following command to run the full build and generate Serenity reports:

```
$ mvn clean verify
```

The tests will run using Chrome by default. To run them in Firefox, override the `driver` system property:
...
Since the Serenity reports contain aggregate information about all of the tests, they are generated once all tests complete. The `serenity-maven-plugin` is configured in the `pom.xml` to create the full report during the Maven lifecycle. You can also generate Allure reports from the `target/allure-results` directory if Allure is configured.
